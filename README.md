...
Tokens are used to authenticate users and send the right data to the user based on what they are authorised to see and what is relevant to them.
...
Clients send a request to the server, if the request is in the right format and the client is authorised, the server responds with the data that the client requested.
